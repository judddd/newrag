...
### 1. search_content - 搜索实际文本内容 ⭐

向量(70%)+BM25(30%)混合搜索，自动处理embedding。
- 搜索文档中的实际文字、数据、说明
- 适用于概念/语义查询、技术内容查询
- **默认推荐**，适用于大多数搜索需求

### 2. search_description - 搜索页面完整描述 🆕

向量(70%)+BM25(30%)混合搜索，基于 VLM 生成的完整页面描述。
- 查找页面视觉特征和技术细节
- 适用于查找图表类型（电路图、流程图、表格等）
...
### 3. keyword_search - 纯关键词搜索

BM25算法，速度快，适合精确匹配编号、人名等。
...
### 场景1: 查找文档实际内容
使用 `search_content` 工具搜索文档中的实际文字。
...
### 场景2: 查找页面视觉特征
使用 `search_description` 工具查找特定类型的页面。
